...
def load_and_process_data():
    """Load and process the CSV data with caching for better performance."""
    try:
        ...
        if not csv_files:
            st.error("No CSV files found in the raw-data folder!")
            return pd.DataFrame()
        ...
        required_columns = ['Amount spent (USD)', 'Ad name', 'Impressions', 'Link clicks', 'Results', 'Purchases conversion value', '3-second video plays']
        ...
        if 'Result type' in df.columns:
            df = df.drop('Result type', axis=1)
